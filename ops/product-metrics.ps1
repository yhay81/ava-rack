...
$Sql = "SELECT COUNT(DISTINCT CASE WHEN is_qa = 0 THEN user_hash END) AS users, COUNT(CASE WHEN is_qa = 0 THEN 1 END) AS events, COUNT(CASE WHEN is_qa = 1 THEN 1 END) AS qa_rows FROM events;"

$Output = & $Wrangler d1 execute ava-rack $Target --command $Sql --json
