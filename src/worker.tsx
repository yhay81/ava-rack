...
import { AboutPage, HomePage, NotFoundPage, PrivacyPage } from "./ui/pages";
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${PRODUCT.origin}/about</loc></url>
...
app.post("/api/events", async (context) => {
  ...
  const clientId = context.req.header("X-Ava-Client") ?? "";
  ...
  await context.env.DB.prepare("INSERT INTO events (user_hash, event_name, is_qa) VALUES (?, ?, ?)")
    .bind(userHash, body.event, context.req.header("X-Ava-QA") === "1" ? 1 : 0)
    .run();
